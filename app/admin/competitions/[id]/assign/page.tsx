...
export default function AssignCourts({ params }: { params: Promise<{ id: string }> }) {
  const [compId, setCompId] = React.useState<string>("");

  // resolve params asynchronously
  React.useEffect(() => {
    params.then(p => setCompId(p.id));
  }, [params]);

  const { data: meta } = useSWR(compId ? `/api/competitions/${compId}` : null, fetcher);
  const { data: teams } = useSWR(compId ? `/api/competitions/${compId}/teams` : null, fetcher);
    ...
    <DashboardShell>
      ...
      <Box sx={{ display: 'grid', gap: 2, maxWidth: 720 }}>
        <TextField select label="Round" value={form.round || ''} onChange={e => setForm({ ...form, round: e.target.value })}>
          {meta?.rounds?.filter((r: any) => r.enabled).map((r: any) => (
        ...
        <TextField select label="Court Number" value={form.courtNumber || ''} onChange={e => setForm({ ...form, courtNumber: Number(e.target.value) })}>
          {Array.from({ length: meta?.numberOfCourtRooms || 0 }, (_, i) => (
