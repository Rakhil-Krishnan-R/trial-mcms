...
export default function JudgeLogin() {
  ...
  const login = async () => {
    const res = await fetch('/api/judge/login', { method: 'POST', body: JSON.stringify({ courtId, judgeId, password }) });
    if (res.ok) window.location.href = `/judge/scoresheet?courtId=${courtId}&judgeId=${judgeId}`;
    else alert('Login failed');
  };

  return (
    <DashboardShell>
      <Typography variant="h5" sx={{ mb: 2, fontWeight: 700 }}>Judge — Session Login</Typography>
      <Box sx={{ display: 'grid', gap: 2, maxWidth: 720 }}>
        ...
        {meta && (<>
          ...
          <Typography>Petitioner: {meta.petitioner?.teamCode} | Respondent: {meta.respondent?.teamCode}</Typography>
          <TextField select label="Judge Name" value={judgeId} onChange={e=>setJudgeId(e.target.value)}>
            {judges.map((j:any)=>(<MenuItem key={j.judgeId} value={j.judgeId}>{j.name}</MenuItem>))}
          </TextField>
